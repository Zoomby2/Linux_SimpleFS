...
/* параметры загружаемого модуля ядра */
...
/* Вычисление контрольной суммы для подтверждения целостности суперблока */
...
static long simplefs_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SIMPLEFS_IOC_ZERO_FILES:
            /* Поочередное зануление для всех десяти файлов */
            for (i = 0; i < 10; i++) {
        ...
        case SIMPLEFS_IOC_ERASE_FS:
            /* Безвозвратное затирание основного и резервного суперблоков */
        ...
        case SIMPLEFS_IOC_GET_HASHES: {
            ...
            /* Генерация уникальных хэшей на основе параметров размещения файлов */
            for (idx = 0; idx < 10; idx++) {
                char fname[32];
                __u64 start_sector = sb_second_offset + 1 + (idx * max_file_sectors);
                __u64 sector_count = max_file_sectors;
                uint32_t sum = 0;
                int k;

                snprintf(fname, sizeof(fname), "file%d", idx);
                /* Алгоритм подсчета суммы байт имени файла и номеров секторов */
                for (k = 0; fname[k] != '\0'; k++) {
                    sum += fname[k];
                }
                ...
                hashes[idx] = 0x12340000 + sum;
            }

            /* Безопасное копирование всего массива хэшей в пространство пользователя */
...
/* Чтение данных из выбранного индексного дескриптора */
...
/* Набор операций для управления каталогами */
...
/* Привязка функции поиска к операциям индексного дескриптора каталога */
static const struct inode_operations simplefs_dir_inode_operations = {
    .lookup = simplefs_lookup,
};

/* Перечень общих операций суперблока */
static const struct super_operations simplefs_s_ops = {
    .statfs = simple_statfs,
    .drop_inode = generic_delete_inode,
};

/* Чтение, валидация и разбор метаданных диска при монтировании */
static int simplefs_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    bh = sb_bread(sb, sb_first_offset);
    if (!bh) {
        printk(KERN_ERR "Не удалось прочитать основной суперблок %d\n", sb_first_offset);
        return -EIO;
    }
    ...
    if (le32_to_cpu(disk_sb->magic) != SIMPLEFS_MAGIC || le32_to_cpu(disk_sb->hash) != calc_hash) {
        ...
        bh = sb_bread(sb, sb_second_offset);
        if (!bh) {
            printk(KERN_ERR "Не удалось прочитать резервный суперблок\n");
            return -EIO;
        }
...
/* Регистрация точки монтирования блочного устройства */
...
/* Описание параметров регистрируемого типа файловой системы */
...
/* Точка входа при инициализации модуля ядра */
static int __init simplefs_init(void)
{
    return register_filesystem(&simplefs_type);
}

/* Точка выхода при выгрузке модуля ядра */
